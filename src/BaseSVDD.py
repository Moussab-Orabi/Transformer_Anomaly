...
class BaseSVDD(BaseEstimator, OutlierMixin):
    ...
    def __init__(self, C=0.9, kernel="rbf", degree=3, gamma=None, coef0=1, display="on", n_jobs=None):
        self.C = C
        self.kernel = kernel
        self.degree = degree
        self.gamma = gamma
    ...
    def solve_problem(self, K):
        ...
        self.radius = np.sqrt(
            np.mean(np.diag(K)[self.boundary_indices]) + self.offset + np.mean(tmp_[self.boundary_indices, 0])
        )
    ...
    def _check_X_y(self, X, y):
        ...
        if np.all(tmp_ == np.array([1])) or np.all(tmp_ == np.array([-1])):
            ...
            errorText = (
                "SVDD is only supported for one-class or binary classification. "
    ...
    def plot_distance(self, radius, distance):
        ...
        ax.legend(["Radius", "Distance"], ncol=1, loc=0, edgecolor="black", markerscale=1, fancybox=True)
        ax.yaxis.grid()
        plt.show()

    def plot_boundary(self, X, y=None, expand_ratio=0.2, n_grids=50, color_map="RdYlBu", n_level=6):
        """
        DESCRIPTION
        ...
        if self.y_type == "single":
            ax3.scatter(X[:, 0], X[:, 1], facecolor="C0", marker="o", s=100, linewidths=2, edgecolor="black", zorder=2)

            ax3.scatter(
                X[self.support_vector_indices, 0],
            ...
            ax3.contour(xv, yv, distance, levels=[self.radius], colors="C3", linewidths=7, zorder=1)

            ax3.legend(
                ["Data", "Support vectors"], ncol=1, loc="upper left", edgecolor="black", markerscale=1.2, fancybox=True
            )
...
class BananaDataset:
    ...
    def split(data, label, **kwargs):
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            data, label, test_size=ratio, random_state=None, shuffle=True, stratify=label
        )
        pIndex = y_train == 1
        nIndex = y_train == -1
